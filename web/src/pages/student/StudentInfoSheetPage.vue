...
// ---------------------------------------------------------- duty schedule
//
// ONE string under `ojt_info.intern_duty_schedule`, TYPED AND NOTHING ELSE.
// Separate shift-time fields are deferred on this project and must not be
// introduced here.
//
// **THE DAY/TIME DROPDOWN BUILDER WAS REMOVED ON 2026-09-08 at the project
// owner's request — do not bring it back.** It offered six fixed day ranges and
// half-hourly times behind a "Type it instead / Use dropdowns" toggle, and paid
// for that with ~170 lines: a compose function, a parser that had to round-trip
// every partial the composer could emit, a suppression flag so prefilling could
// not write back over the value it had just read, a mode that had to be chosen
// on load, and a save button disabled whenever the picked range was impossible.
...
// not be selected, and the toggle stood between the student and the text box
// they needed anyway.
//
// THE STORED SHAPE IS UNCHANGED (`nullable|string|max:150`), so every value
// already saved — however it was composed — still loads and prints exactly as
// before. The placeholder keeps the old format's EN DASHes (U+2013), now as a
// suggestion rather than a grammar.
    ...
    <form class="rounded-lg bg-white p-6 shadow-sm ring-1 ring-slate-200" @submit.prevent>
      ...
      <div class="mt-5 rounded-lg bg-slate-50 p-5">
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <!--
            Free text, and only free text. The dropdown builder that used to
            sit here was removed 2026-09-08 — see the duty schedule note in
            the script block for why it must not come back. maxlength mirrors
            StoreInfoSheetRequest's max:150 so the cap is felt while typing
            rather than as a 422 on save.
          -->
          <label class="block text-sm font-medium text-slate-700 md:col-span-2">
